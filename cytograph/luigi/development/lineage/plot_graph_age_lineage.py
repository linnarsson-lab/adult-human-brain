...
class PlotGraphAgeLineage(luigi.Task):
	...
	def output(self) -> luigi.Target:
		if self.time == "E7-E18":  # This is for backwards comaptibility we might remove this condition later
			return luigi.LocalTarget(os.path.join(cg.paths().build, self.lineage + "_" + self.target + ".mknnage.png"))
		else:
			return luigi.LocalTarget(os.path.join(cg.paths().build, "%s_%s_%s.mknnage.png" % (self.lineage, self.target, self.time)))
