...
class HPF:
	...
	"""
	def __init__(self, k: int, a: float = 0.3, b: float = 0.3, c: float = 0.3, d: float = 0.3, max_iter: int = 1000, stop_interval: int = 10, stop_at_ll: float = 0.000001, n_threads: int = None) -> None:
		self.k = k
		self.a = a
		self.b = b
		self.c = c
	...
	def _fit(self, X: sparse.coo_matrix, beta_precomputed: bool = False) -> Tuple[np.ndarray, np.ndarray]:
		...
		if self.n_threads is not None:
			previous_setting = numexpr.set_num_threads(int(self.n_threads))
		...
		kappa_shape = np.full(n_users, a, dtype='float32') + np.random.uniform(0, 0.1, n_users)
		kappa_rate = np.full(n_users, b + k, dtype='float32')
		gamma_shape = np.full((n_users, k), a, dtype='float32') + np.random.uniform(0, 0.1, (n_users, k))
		gamma_rate = np.full((n_users, k), b, dtype='float32') + np.random.uniform(0, 0.1, (n_users, k))
		...
		if beta_precomputed:
			...
			tau_shape = np.full(n_items, c, dtype='float32') + np.random.uniform(0, 0.1, n_items)
			tau_rate = np.full(n_items, d + k, dtype='float32')
			lambda_shape = np.full((n_items, k), c, dtype='float32') + np.random.uniform(0, 0.1, (n_items, k))
			lambda_rate = np.full((n_items, k), d, dtype='float32') + np.random.uniform(0, 0.1, (n_items, k))
		...
		while True:
			...
			if n_iter % self.stop_interval == 0:
				...
				if len(self.log_likelihoods) > 1:
					...
					# The log likelihood is falling, stop after three such events
					if log_likelihood < prev_ll:
						n_ll_falling += 1
						if n_ll_falling > 2:
							break
					else:
						# The log likelihood is increasing but how slowly?
