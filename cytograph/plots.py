...
def plot_markerheatmap(ds: loompy.LoomConnection, dsagg: loompy.LoomConnection, n_markers_per_cluster: int = 10, out_file: str = None) -> None:
	...
	fig = plt.figure(figsize=(30, 2.5 + n_tissues / 5 + n_classes / 5 + n_topmarkers / 10))
	gs = gridspec.GridSpec(2 + n_tissues + n_classes + 1, 1, height_ratios=[50, 1] + [1] * n_tissues + [1] * n_classes + [0.5 * n_topmarkers])
...
#    ax.set_yticks([])
	...
	for ix, t in enumerate(tissues):
		...
		ax.imshow(np.expand_dims((ds.col_attrs["Tissue"] == t).astype('int'), axis=0), aspect='auto', cmap="bone", vmin=0, vmax=1)
		ax.set_frame_on(False)
		ax.set_xticks([])
		ax.set_yticks([])
	...
	for ix, cls in enumerate(classes):
		ax = fig.add_subplot(gs[2 + n_tissues + ix])
		ax.imshow(np.expand_dims(ds.col_attrs[cls], axis=0), aspect='auto', cmap="bone", vmin=0, vmax=1)
		ax.set_frame_on(False)
		ax.set_xticks([])
		ax.set_yticks([])
