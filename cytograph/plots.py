...
def plot_markerheatmap(ds: loompy.LoomConnection, dsagg: loompy.LoomConnection, n_markers_per_cluster: int = 10, out_file: str = None) -> None:
	...
	genes = [g for g in genes if g in ds.col_attrs["Gene"]]
	n_genes = len(genes)
	...
	for ix, g in enumerate(genes):
		...
		gix = np.where(ds.col_attrs["Gene"] == g)[0][0]
		vals = ds[gix, :][cells]
	...
	for ix in range(n_topmarkers):
		...
		text = plt.text(0.001 + xpos, ix - 0.5, ds.col_attrs["Gene"][:n_markers][ix], horizontalalignment='left', verticalalignment='top', fontsize=4, color="white")
