...
The accelerated version below is broken, and does not yield the same result as the pure Python version. There's something
wrong about the normalization probably.
"""
...
class HPF_accel:
	...
	def __init__(self, k: int = 100, a: float = 1, b: float = 5, c: float = 1, d: float = 5, 
		...
		for attr, value in numexpr_kwargs.items():
			getattr(numexpr, attr)(value)
			logging.debug(f"Calling - numexpr.{attr}({value})")
