...
class PrincipalCurve:
	...
	Untested
	"""
	def __init__(self, tolerance: float = 0.0001, d: int = 1, h: int = 18, maxiter: int = 600) -> None:
	...
	def _projectpoint(self, data: np.ndarray, point: np.ndarray) -> np.ndarray:
		...
		while not(converged):
			...
			V = np.reshape(v[:, index[range(n - self.d)]], (n, self.d))
			ospace = np.dot(V, V.T)
	...
	def _hess(self, data: np.ndarray, x: np.ndarray) -> float:
		...
		cs = self._c(data, x)
		us = self._u(data, x)
		Hx = sum(cs * ((us[:, newaxis, :] * us) - Sigmainv[:, :, newaxis]), 2) / N
		return Hx
	...
	def _p(self, data: np.ndarray, points: np.ndarray) -> np.ndarray:
		...
		for i in range(M):
			diff = (data - points[:, i, np.newaxis]) / self.h
